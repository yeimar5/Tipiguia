...
function copiarEnTipificar() {
    ...
    switch (motivoLlamada) {
        case `1`: // agendar
            ...
            texto += notaGenerada + gestion;
            break;

        case `2`: // quiebre
            if (contingenciaActiva) {
                texto = `QC - ${motivoQuiebre}-${texto} no se marca al número ${numeroTitular} titular ${nombreTitular} POR CONTINGENCIA se deja orden suspendida en aplicativos${gestion}`;
                ...
                texto = `QC - ${motivoQuiebre}-${texto} Se marca al número ${numeroTitular} titular ${nombreTitular} ${motivoCliente} se hace objeción pero desiste valida datos, se procede a quebrar orden ${gestion}`;
            } else if (contactoConTitular == `2` && motivoQuiebre != `TELEFONO DEL CLIENTE ERRADO` && motivoQuiebre != `GESTIÓN COMERCIAL/CLIENTE ACEPTA INSTALACIÓN` && motivoQuiebre != `GESTIÓN COMERCIAL/CLIENTE SOLICITA LLAMAR EN 10 MIN` && suspenderOrden) {
                texto = `QC - ${motivoQuiebre}-${texto} Se marca al número ${numeroTitular} titular ${nombreTitular} ${motivoCliente} se deja orden suspendida en aplicativos${gestion}`;
                ...
                texto = `LINEA RESCATE Se comunica técnico  informando Titular desea cancelar el servicio ${motivoTecnico} Se marca al número ${numeroTitular} titular ${nombreTitular} ${motivoCliente} se hace objeción acepta instalación, valida datos correctos${gestion}`;
                ...
                texto = `QC - ${motivoQuiebre}-${texto}se marca al número ${numeroTitular} titular ${nombreTitular} ${motivoCliente} se valida SOPORTE FOTOGRÁFICO OK se valida GPS OK se procede a suspender orden${gestion}`;
                ...
                if (gpsActivo == `OK` && soporteFotografico == `OK` && motivoQuiebre != `INCUMPLIMIENTO DE CITA ATRIBUIBLE A TITULAR`) {
                    texto = `QC - ${motivoQuiebre}-${texto} Se marca al número ${numeroTitular} titular ${nombreTitular} No contesta Se Valida GPS ${gpsActivo} Se Valida SOPORTE FOTOGRÁFICO ${soporteFotografico} se procede a  suspender orden${gestion}`;
                } else if (motivoQuiebre == `INCUMPLIMIENTO DE CITA ATRIBUIBLE A TITULAR` && gpsActivo == `OK` && soporteFotografico == `OK`) {
                    texto = `QC - ${motivoQuiebre}-${texto}se marca al número ${numeroTitular} titular ${nombreTitular} no contesta se valida SOPORTE FOTOGRÁFICO ${soporteFotografico}  se valida GPS ${gpsActivo} , YA QUE LLEVA MAS DE TRES VECES DE NO CONTACTO CON CLIENTE SE  PROCEDE A QUEBRAR ORDEN${gestion}`;
                } else {
                    texto += `se marca al número ${numeroTitular} titular ${nombreTitular} No contesta Se Valida GPS ${gpsActivo} Se Valida SOPORTE FOTOGRÁFICO ${soporteFotografico} Se le indica a Tecnico dirigirse al predio y Subir Soporte fotográfico${gestion}`;
                }
            }

            if (contactoConTitular == `1` && trabajador == `gestor`) {
                texto += `se marca al número ${numeroTitular} titular ${nombreTitular} no contesta se le indica a gestor que favor envié técnico a predio para proceder con la gestión${gestion}`;
        ...
        case `3`: // soporte no aplica
            ...
            texto += mensajes[soporteNoAplica] + gestion;

            break;
        case `4`: // Gestion de decos
            texto += ` Se marca al número ${numeroTitular} titular ${nombreTitular} ${motivoCliente} se valida datos correctos y se actualiza TAG de equipos${gestion}`;
            break;
        case `5`: // Direccion piloto
            ...
            texto +=  ` aceptación de línea de rescate ` + respuesta + gestion;
            break;
        case `6`:
            texto += ` pero se cae la llamada sin poder validar la información` + gestion; 
            break;
    }
