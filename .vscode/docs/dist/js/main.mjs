...
import * as DireccionPiloto from "./modulos/modulo-direccion-piloto.mjs";
import * as Agendar from "./modulos/modulo-agendar.mjs";
import * as Incumplimiento from "./modulos/modulo-incumplimiento.mjs";
import * as Quiebre from "./modulos/modulo-quiebre.mjs";
import * as noSoporte from "./modulos/modulo-soporteNA.mjs";
import * as Decos from "./modulos/modulo-gestionDecos.mjs";
import * as Sesion from "./modulos/modulo-sesion.mjs";
import * as Utilidades from "./modulos/modulo-utilidades.mjs";
import * as CreacionNota from "./modulos/modulo-creacion-nota.mjs";
import * as Validacion from "./modulos/modulo-validacion.mjs";
...
function manejarClick(evento) {
  ...
  const actions = {
    copiar: () =>
      Utilidades.copiarYAlertar(document.getElementById(`NombreTec`).value, Utilidades.alerta),
    copiar1: () =>
      Utilidades.copiarYAlertar(document.getElementById(`cedula`).value, Utilidades.alerta),
    copiar2: () => {
      let texto = document.getElementById(`telefono`).value;
      Utilidades.copiarYAlertar(texto, Utilidades.alerta);
    },
    copiar3: () =>
      Utilidades.copiarYAlertar(document.getElementById(`atis`).value, Utilidades.alerta),
    btnCopiarNota: copiarNotaConValidacion,
    pedirCuota: pedirCuota,
    limpiar: limpiarFormularios,
    Tipificar: lanzarModal,
    imagen: Sesion.subirImagen, 
...
function concatenateInputs() {
  ...
  const parts = [via, complemento].filter((part) => part.trim() !== "");
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  aplicarLimpiezaAInput(via, Utilidades.limpiarTexto); */
...
function aplicarLimpiezaAInput(elementoInput, funcionLimpieza) {
  if (!elementoInput || !elementoInput.nodeType) {
    console.warn("Se requiere un elemento DOM válido");
    return;
  }

  if (typeof funcionLimpieza !== "function") {
    console.warn("Se requiere una función de limpieza válida");
    return;
  }

  function aplicarLimpieza() {
    const textoOriginal = elementoInput.value;
    const textoLimpio = funcionLimpieza(textoOriginal);

    if (textoOriginal !== textoLimpio) {
      const cursorPos = elementoInput.selectionStart;
      elementoInput.value = textoLimpio;
      const nuevaPos = Math.min(cursorPos, textoLimpio.length);
      elementoInput.setSelectionRange(nuevaPos, nuevaPos);
    }
  }

  // Agregar event listeners
  elementoInput.addEventListener("paste", () => setTimeout(aplicarLimpieza, 0));
  elementoInput.addEventListener("input", aplicarLimpieza);
}
